...
//        add_filter('get_items_with_flag', array(__CLASS__, "item_filter"), 10, 1);
        ...
        function check_post_id_callback() {            
            ...
            $settings = $forms_model->getFormCustomField($form_id, 'form_settings');
            if (isset($myform) &&
                    isset($settings) &&
                    isset($myform->post_type) &&
                    isset($settings->form['type']) &&
                    $myform->post_type == CRED_FORMS_CUSTOM_POST_NAME &&
                    $settings->form['type'] == 'new') {
                $post_id = intval($_POST['post_id']);
                $mypost = get_post($post_id);

                if (isset($mypost) && $mypost->post_status != 'auto-draft') {
                    global $wpdb;
