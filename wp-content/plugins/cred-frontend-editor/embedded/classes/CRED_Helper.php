...
// On what admin pages should CRED assets be loaded?
        $set_on_pages = array('view-archives-editor', 'views-editor', 'CRED_User_Forms', 'CRED_Forms', 'CRED_Fields', 'CRED_User_Fields', 'CRED_Settings', 'CRED_Help');
...
// Inline Settings/Localization
            wp_localize_script('cred_cred', 'cred_settings', array(
                ...
                'autogenerate_username_scaffold' => isset($form_fields) ? $form_fields['form_settings']->form['autogenerate_username_scaffold'] : 0,
                'autogenerate_nickname_scaffold' => isset($form_fields) ? $form_fields['form_settings']->form['autogenerate_nickname_scaffold'] : 0,
                'autogenerate_password_scaffold' => isset($form_fields) ? $form_fields['form_settings']->form['autogenerate_password_scaffold'] : 0,
                // settings
    ...
    public static function accessActiveFormAlertNotice() {
        ?><div class="cred-notification cred-error" style="margin-right:15px;">
            ...
            <p>
                <?php printf(__('To control who can see and use this form, go to the %s', 'wp-cred'), '<a href="' . admin_url() . '?page=types_access">Access</a>'); ?>
                <?php
...
// format the data for the view/model data
        ...
        elseif (self::$currentPage->isPostEdit || self::$currentPage->isPostNew ||
                ...
                $screen->id == 'views-new_page_views-editor' || $screen->id == 'views-new_page_view-archives-editor' || //DEPRECATED
                $screen->id == '‘toolset_page_views-editor' || $screen->id == 'toolset_page_view-archives-editor'
		) {
            ?>
            <script type='text/javascript'>
                /*<![CDATA[ */
...
// provide WPML localization for hardcoded texts
    ...
    public static function cred_edit_user_link($form, $post_id = false, $text = '', $class = '', $style = '', $target = '', $attributes = '') {
        global $post, $current_user;
...
//return '<strong>'.__('Do not have permission (edit other with this form)','wp-cred').'</strong>';
        ...
        $link = esc_url(add_query_arg(array('cred-edit-user-form' => $form), $link));
        $link = esc_url(add_query_arg(array('user_id' => $post_id), $link));
// provide WPML localization for hardcoded texts
...
// else do nothing
    ...
    public static function credUserFormLinkShortcode($atts) {
        ...
        return self::cred_edit_user_link($params['form'], $params['post'], $params['text'], $params['class'], $params['style'], $params['target'], $params['attributes']);
    }
