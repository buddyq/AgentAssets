...
if (isset($data['options'])) {
    foreach ($data['options']['value'] as $ii => $option) {
        ?>
                    ...
                    <span class='cell'>
        <?php
        if (isset($data['options']['option_default']) &&
                (
...
<?php if ($field['type'] != 'hidden') { ?>
    ...
    <?php //if ($post_type != 'cred-user-form') { ?>
                                        <li>
                                                <label class='cred-label'>
                                                        <input type='checkbox' class='cred-checkbox' name='field[include_scaffold]' value='1' <?php if (isset($data['include_scaffold']) && $data['include_scaffold']) echo 'checked="checked"'; ?> />
                                    <span class='label'><?php _e('Include this field in Scaffold', 'wp-cred'); ?></span>
            ...
            (function (window, $, undefined) {
                $(function () {
                    if (<?php echo $popup_close; ?>)
                    {
                        ...
                        var field_type_title = '<?php if (isset($field['title'])) echo esc_js($field['title']);
else echo esc_js(__('Not Set', 'wp-cred')); ?>';
...
<?php
$txt = ($post_type == 'cred-user-form') ? "User" : "";
?>
    <body id='cred_generic_fields' class="wp-core-ui">
        <div class='cred-header'><i class="icon-cred-logo ont-icon-32"></i><?php _e('Custom ' . $txt . ' Field Setup', 'wp-cred'); ?>&nbsp;&nbsp;(<?php echo $field['title']; ?>)</div>
