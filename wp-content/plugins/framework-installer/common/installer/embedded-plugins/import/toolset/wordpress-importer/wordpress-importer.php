 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
	class WP_Lions_Import extends WP_Importer {
		...
		function process_attachment( $post, $url ) {
			...
			$old_url = $url;
			$new_url = preg_replace( "/.*\/uploads/", '', $url );
			$new_url = get_stylesheet_directory_uri() . '/toolset_import/media' . $new_url;
			$url     = $new_url;
...
function lions_wordpress_importer_init() {
	 ...
	 */
	$GLOBALS['wp_import'] = new WP_Lions_Import();
	register_importer( 'wordpress', 'WordPress', __( 'Import <strong>posts, pages, comments, custom fields, categories, and tags</strong> from a WordPress export file.', 'wordpress-importer' ), array(
		$GLOBALS['wp_import'],
		'dispatch'
	) );
}

add_action( 'admin_init', 'lions_wordpress_importer_init' );
