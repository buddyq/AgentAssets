...
class WPDD_PostEditPageManager
{
    ...
    function page_edit_scripts() {
        ...
        if( $wpddlayout->theme_has_page_templates() ){
            ...
            $wpddlayout->localize_script('ddl_post_edit_page', 'DDLayout_settings_post_edit', array(
                'strings' => array(
        ...
        if ($_POST && isset($_POST['action']) && in_array( $_POST['action'], self::$FORBIDDEN_ACTIONS ) === false ) { // Don't save in quick edit mode.
            ...
            if( $layout_template ){
                ...
                else
                {
                    ...
                    WPDD_Utils::assign_layout_to_post_object( $pidd,  $layout_selected, $tpl  );
                }
        ...
        ?>
            <div class="create-layout-for-page-wrap hidden">
                ...
                if( class_exists( 'WPDDL_Admin_Pages') ){
                    add_action('admin_footer', array(WPDDL_Admin_Pages::getInstance(),'include_dialog_template_for_cpts'));
                }
                endif;?>
            </div>
        <?php
