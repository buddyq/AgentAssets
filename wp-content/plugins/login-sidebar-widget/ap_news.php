...
if (!function_exists('ap_news_script')) {
	function ap_news_script(){ 
    	echo '<script type="text/javascript">
			function ap_load_news(){jQuery.ajax({method:"POST",beforeSend:function(){jQuery("#ap_news_feed").html("loading..")},data:{ap_news_option:"LoadAPNews"}}).done(function(e){jQuery("#ap_news_feed").html(e)})}
        </script>';
	}
}
...
if (!function_exists('ap_news_news_loader')) {
	function ap_news_news_loader(){
		if( isset($_REQUEST['ap_news_option']) and $_REQUEST['ap_news_option'] == 'LoadAPNews' ){
					
			if(!isset($_SESSION['ap_news']) or empty($_SESSION['ap_news'])){
