...
class login_wid extends WP_Widget {
	...
	public function add_extra_links(){
		...
		if($login_afo_forgot_pass_link){
			echo '<a href="'.get_permalink($login_afo_forgot_pass_link).'">'.__('Lost Password?','login-sidebar-widget').'</a>';
		}
		if($login_afo_register_link){
			echo ' | <a href="'.get_permalink($login_afo_register_link).'">'.__('Register','login-sidebar-widget').'</a>';
		}
	}
	...
	public function loginForm(){
		...
		if($link_in_username){
			$link_with_username = '<a href="'.get_permalink($link_in_username).'">'.__('Howdy','login-sidebar-widget').', '.$current_user->display_name.'</a>';
		} else {
...
function login_validate(){
	if( isset($_POST['option']) and $_POST['option'] == "afo_user_login"){
		...
		if($_POST['user_username'] != "" and $_POST['user_password'] != ""){
			...
			if($user->ID == ""){
				$_SESSION['msg_class'] = 'error_wid_login';
				$_SESSION['msg'] = __('Error in login!','login-sidebar-widget');
			} else{
