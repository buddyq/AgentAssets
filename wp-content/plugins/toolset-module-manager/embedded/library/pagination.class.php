...
<?php
  ...
  *
  ...
  class pagination
  {
    ...
    {
      $this->array   = $array;
      $this->curPage = ($curPage == null ? $this->defaults['library_page']    : $curPage);
    ...
    public function getResults()
    {
      ...
      if (empty($this->curPage) !== false) {
        $this->library_page = $this->curPage; // using the get method
      } else {
        $this->library_page = 1; // if we don't have a page number then assume we are on the first page
      }
      ...
      $this->start = ceil(($this->library_page - 1) * $this->perPage);
    ...
    public function getLinks($params = array())
    {
      ...
      if (($this->pages) > 1) {
        ...
        if ($this->library_page != 1) {
          if ($this->_showFirstAndLast) {
            $plinks[] = ' <a href="?library_page=1'.$queryUrl.'">&laquo;&laquo; First </a> ';
          }
          $plinks[] = ' <a href="?library_page='.($this->library_page - 1).$queryUrl.'">&laquo; Prev</a> ';
        }
        
        // Assign all the page numbers & links to the array
        for ($j = 1; $j < ($this->pages + 1); $j++) {
          if ($this->library_page == $j) {
            $links[] = ' <a class="selected">'.$j.'</a> '; // If we are on the same page as the current item
          } else {
            $links[] = ' <a href="?library_page='.$j.$queryUrl.'">'.$j.'</a> '; // add the link to the array
          }
        }

        // Assign the 'next page' if we are not on the last page
        if ($this->library_page < $this->pages) {
          $slinks[] = ' <a href="?library_page='.($this->library_page + 1).$queryUrl.'"> Next &raquo; </a> ';
          if ($this->_showFirstAndLast) {
            $slinks[] = ' <a href="?library_page='.($this->pages).$queryUrl.'"> Last &raquo;&raquo; </a> ';
          }
        }
