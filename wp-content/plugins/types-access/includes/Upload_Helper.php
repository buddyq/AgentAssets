...
public static function wpcf_access_user_can_upload_files()
{
    ...
    // Enqueue
    add_filter('wpcf_access_exceptions', array(__CLASS__, 'wpcf_access_exceptions_upload_files'),
            10, 4);
    add_filter('types_access_check_override',
            array(__CLASS__, 'wpcf_access_upload_files_check_override'));
        ...
        if (!empty($post) && $post->post_type == 'attachment') {
            ...
            if (strpos($_SERVER['SCRIPT_NAME'], '/wp-admin/upload.php') !== false
                ...
                if (Access_Helper::wpcf_access_is_media_registered()) {
                    ...
                    $capability_requested = Access_Helper::wpcf_access_map_cap($capability_requested,
                            $post_id);
...
public static function wpcf_access_upload_files_check_override($null, $parse_args = array())
{
    ...
    if (isset($parse_args['cap']) && $parse_args['cap'] == 'upload_files') {
        return Access_Helper::wpcf_access_parse_caps((bool) self::wpcf_access_user_can_upload_files(),
                        $parse_args);
    }
    return $null;
}
...
public static function wpcf_access_user_can_upload_files_update()
{
    ...
    self::wpcf_access_user_can_upload_files();
}
        ...
        switch ($action) {
            ...
            case 'types-end-image-upload':
                ...
                $map = map_meta_cap($parse_args['cap'], get_current_user_id(),
                        $parent_id);
                $result = Access_Helper::wpcf_access_check($parse_args['allcaps'], $map,
