		...
		public function show_balance_profile() {
			...
			if ( ! empty( $mycred_types ) ) {
				...
				foreach ( $mycred_types as $type => $label ) {
					...
					$output  .= sprintf( '<div class="bp-widget mycred"><h2>%s</h2><table class="profile-fields"><tr class="field_1 field_current_balance_' . $type . '"><td class="label">%s</td><td class="data">%s</td></tr></table></div>', $mycred->plural(), __( 'Current balance', 'mycred' ), $mycred->format_creds( $balance ) );
...
?>
<style type="text/css">
.pagination-links { float: right; min-width: 100px;}
.pagination-links > span { float: left;}
