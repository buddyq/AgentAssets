...
function aveone_fields() {
	...
	foreach ($options as $value) {
      ...
      if ( $value['type'] == "open-tab" || $value['type'] == "close-tab") { } else {
		...
		break;
			...
			case 'editor':
				...
				//$output .= wp_editor( $val, $value['id'], $editor_settings );
				// Turn on the output buffer
                                ob_start();

                                // Echo the editor to the buffer
                                wp_editor( esc_attr( $val ), esc_attr( $value['id'] ), $editor_settings );
		...
		// Uploader
		case "upload":
      $output .= aveone_Framework_Media_Uploader::aveone_uploader( $value['id'], $val, null );	
		break;
		...
		case 'typography':	
		  ...
		  $faces = aveone_recognized_font_faces1();	
			...
			/* Font Style */
			$styles = aveone_recognized_font_styles();
      ...
      $output .= aveone_Framework_Media_Uploader::aveone_uploader( $value['id'], $background['image'], null, esc_attr( $option_name . '[' . $value['id'] . '][image]' ) );
			...
			$repeats = aveone_recognized_background_repeat();
			...
			$positions = aveone_recognized_background_position();
			...
			$attachments = aveone_recognized_background_attachment();
	...
	// Info
		case "info":
			...
			if ( $value['desc'] ) {
				$output .= apply_filters('aveone_sanitize_info', $value['desc'] ) . "\n";
			}
    ...
    // Backup
      ...
      $output .= 'Please visit <a href="'.admin_url().'themes.php?page=aveone-options-backup">this page</a> to import / export your theme options';  
  
      break;
  
      case "note-for-printables":
   
      $output .= '<a target="_blank" href="'.admin_url().'edit.php?post_type=printable_info">Click Here</a> to add attachments to Printable Info';  
  
      break;
  
      case "note-for-social-media":
   
      $output .= '<a target="_blank" href="http://agentassets.com/edit-my-profile/">Click Here</a> to add Social Media Information';  
  
      break;
  
      case "note-for-agent-information":
   
      $output .= '<a target="_blank" href="http://agentassets.com/edit-my-profile/">Click Here</a> to add Agent Information';  
